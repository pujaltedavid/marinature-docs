...
As seen in the animation above, the user can create, edit and delete collections easily. Each collection card has a menu in the upper right that allows to edit the title, hide or show the collection to the public and delete it. However, for safety reasons, the user can not delete a collection if it contains photos. They would have to delete the images first, and then delete the collection. If the user does not want to make a collection visible, for example because they are still uploading the images and completing the descriptions, they can just hide that collection.
